    ...
    internal sealed class DnnRetrainTransformer : RowToRowTransformerBase, IDisposable
    {
        ...
        private (int, bool, TF_DataType, Shape) GetTrainingInputInfo(DataViewSchema inputSchema, string columnName, string tfNodeName, int batchSize)
        {
            ...
            if (numInputDims != -1)
            {
                ...
                newShape[0] = dims[0] == 0 || dims[0] == -1 ? batchSize : (int)dims[0];
        ...
        internal static (TF_DataType[] tfOutputTypes, DataViewType[] outputTypes, (Operation, int)[]) GetOutputInfo(IHost host, Session session, string[] outputs)
        {
            ...
            for (int i = 0; i < outputs.Length; i++)
            {
                ...
                int[] dims = shape.ndim > 0 ? shape.dims.Skip(shape.dims[0] == -1 ? 1 : 0).Select(item => (int)item).ToArray() : new[] { 0 };
        ...
        private class TensorValueGetter<T> : ITensorValueGetter
        {
            ...
            public Tensor GetBufferedBatchTensor()
            {
                if (_keyType)
                {
                    var tensor = new Tensor(_bufferedDataLong, new Shape(_dims));
                    _position = 0;
                    return tensor;
                }
                else
